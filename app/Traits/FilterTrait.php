...
trait FilterTrait {
	...
	public function scopeGetData($query, $count, $input = null) {
		...
		if($only && is_array($only)) { $only = array_filter($only); }
		if((int)$all < 1 && (boolean)$array) {
	...
	public function scopeSelectColumnFilter($query, $input = null) {
		...
		if($selectColumn === null || $selectColumn === '') { return $query; }
    ...
    public function scopeOrderByFilter($query, $input = null, $tableName = null) {
        ...
        if($orders && count($orders) > 0) {
	        ...
	        $columnArray = DB::getSchemaBuilder()->getColumnListing($this->getTable());
	...
	public function scopeSlugFilterOn($query, $input = null, $columnName = null, $withReplaces = true, $tableName=null) {
		...
		if($columnName === null || $columnName === '') { return $query; }
		...
		if ($scopeValue && !is_array($scopeValue)) {
			...
			foreach ($scopeValue as $key => $value) {
				$scopeValue[$key] = Str::slug($value, '-');
			}
		} else {
...
///*** RAW Table Record Filters *** /////
	...
	public function scopeBooleanRawFilterOn($query, $input = null, $columnName = null, $tableName = null) {
		...
		if (!$columnName || $columnName === '' || !$tableName) {
			...
			$scopeValue = (Arr::has($input, $columnName) && $input[$columnName] !== null) ? (boolean) json_decode(strtolower($input[$columnName]), true) : null;
			return $query->when(($scopeValue !== null), function ($query) use ($scopeValue, $columnName, $tableName) {
    ...
    public function scopeNearbyFilterOn($query, $input = null, $tableName=null) {
        ...
        if($nearby && $nearby > 0) {
	        ...
	        if(!($latitude && $longitude)) { throw new ErrorResponse('Required Current Location latitude and longitude',405,'info'); }
	        ...
	        return $query->when(($nearby && $nearby > 0), function($q) use ($nearby, $minLat, $maxLat, $minLon, $maxLon, $tableName) {
                if($minLat && $maxLat && $minLon && $maxLon) {
                	return $q->whereBetween('latitude', [$minLat, $maxLat])
