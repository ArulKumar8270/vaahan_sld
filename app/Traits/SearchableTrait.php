...
trait SearchableTrait
{
    ...
    /**
     * Returns the tables that are to be MorphJoined.
     *
     * @return array
     */
    protected function getMorphJoins()
    {
        return Arr::get($this->searchable, 'morphJoins', []);
    }

    /**
     * Adds the sql MorphJoins to the query.
     *
     * @param \Illuminate\Database\Eloquent\Builder $query
     */
    protected function makeMorphJoins(Builder $query)
    {
        // $table = 'resource';
        // $keys = ['11_whatsapp', 'from'];
        // $query->leftJoin($table, function ($join) use ($table, $keys) {
        //     $join->on($table.'.'.$keys[1], '=', $keys[0].'.id')
        //         ->whereRaw($table.'.'.$keys[2] . ' = "' . $keys[0] . '"');
        // });

        foreach ($this->getMorphJoins() as $table => $keys) {
            $query->leftJoin($table, function ($join) use ($table, $keys) {
                $join->on($table.'.'.$keys[1], '=', $keys[0].'.id')
                    ->whereRaw($table.'.'.$keys[2] . ' = "' . $keys[0] . '"');
            });
